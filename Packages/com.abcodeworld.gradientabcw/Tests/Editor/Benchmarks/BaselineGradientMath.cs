// Frozen A/B baseline. This is a verbatim copy of the evaluation and bake code as it stood at the
// end of Phase 1, before any of the Phase 2 optimizations. It exists only so the benchmark fixture
// can measure old against new inside a single run, on the same machine, in the same session --
// numbers compared across separate runs are not trustworthy enough to justify keeping or dropping
// an optimization. Like LegacyGradientOracle, it must never be "improved": the moment it drifts it
...
{
    internal static unsafe class BaselineGradientMath
    {
        ...
        public static float4 SampleAt(in NativeGradient native, float t, in GradientLutOptions options)
        {
            float4 c = options.includeModulation ? Evaluate(in native, t) : EvaluateBase(in native, t);
            if (options.toLinear)
                c = new float4(SrgbToLinear(c.xyz), c.w);
            return c;
        }

        public static Color32 ToColor32(float4 c) => new Color32(
